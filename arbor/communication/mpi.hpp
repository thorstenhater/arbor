...
template <typename T>
inline partitioned_vector<T> all_to_all_impl(const std::vector<T>& send_buffer, const std::vector<int>& send_counts,
...
/// Retains the meta data (i.e. vector partition)
template <typename T>
partitioned_vector<T> all_to_all_with_partition(const partitioned_vector<T>& values, MPI_Comm comm) {
    using traits = mpi_traits<T>;
...
/// Retains the meta data (i.e. vector partition)
template <typename T>
partitioned_vector<T> all_to_all_with_partition(const std::vector<std::vector<T>>& values, MPI_Comm comm) {
    using traits = mpi_traits<T>;
