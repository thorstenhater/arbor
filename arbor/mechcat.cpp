...
ARB_ARBOR_API const mechanism_catalogue load_catalogue(const std::filesystem::path& fn) {
    ...
    if (!get_catalogue) {
        throw bad_catalogue_error{std::format("Unusable symbol 'get_catalogue' in shared object '{}'", fn.string())};
    }
    ...
    if (count <= 0) {
        throw bad_catalogue_error{std::format("Invalid mechanism count {} in shared object '{}'", count, fn.string())};
    }
    mechanism_catalogue result;
    for(int ix = 0; ix < count; ++ix) {
        auto type = mechs[ix].type();
        auto name = std::string{type.name};
        if (name.empty()) throw bad_catalogue_error{std::format("Empty name for mechanism in '{}'", fn.string())};
        auto icpu = mechs[ix].i_cpu();
        auto igpu = mechs[ix].i_gpu();
        if (!icpu && !igpu) {
            throw bad_catalogue_error{std::format("Empty interfaces for mechanism '{}'", name)};
        }
