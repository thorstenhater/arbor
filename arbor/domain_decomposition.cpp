...
domain_decomposition::domain_decomposition(const recipe& rec,
    ...
    gid_rank_.resize(global_gids.size());
    gid_indx_.resize(global_gids.size());
    ...
    for (auto rank: count_along(rank_part)) {
        ...
        for (auto gid: util::subrange_view(global_gids.values(), rank_part[rank])) {
            gid_rank_[gid] = rank;
            gid_indx_[gid] = index_on_domain;
            ++index_on_domain;
        }
    }
}

int domain_decomposition::gid_domain(cell_gid_type gid) const { return gid_rank_[gid]; }

cell_size_type domain_decomposition::index_on_domain(cell_gid_type gid) const { return gid_indx_[gid]; }
...
const group_description& domain_decomposition::group(unsigned idx) const { arb_assert(idx<num_groups()); return groups_[idx]; }
