...
template <typename T, typename I>
__global__
void combined_solve_matrix_fine(T* __restrict__ const rhs,
                                const T* __restrict__ const u,                       // upper diagonal
                                T* __restrict__ const d,                             // mutable part of diagonal
                                const T* __restrict__ const invariant_d,             // fixed part of diagonal
                                T* __restrict__ const voltage,                       // Voltage, in-out argument
                                const T* __restrict__ const current,
                                const T* __restrict__ const conductivity,
                                const T* __restrict__ const cv_capacitance,
                                const T* __restrict__ const area,
    ...
    __syncthreads();
    {
        ...
        if (tid < n) voltage[tid] = rhs[perm[tid]];
    }
}
...
solve_matrix_combined(arb_value_type* voltage,               // in/out: membrane potential
    ...
    launch(cuda_blocks,
           ...
           rhs,
           u,           // upper diagonal
           d,           // mutable part of diagonal
           invariant_d, // fixed part of diagonal
           voltage,     // Voltage, in-out argument
           current,
           conductivity,
           capacitance,
           area,
